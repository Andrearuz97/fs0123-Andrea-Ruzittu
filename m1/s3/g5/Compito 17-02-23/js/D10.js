...
console.log(whoIsBigger(300, 290)); 
...
function whatDayIsIt() {
  const GiorniDellaSettimana = ['Domenica', 'Lunedi', 'Martedì', 'Mercoledì', 'Giovedi', 'Venerdì', 'Sabato'];
  const Oggi = new Date();
  const Indice = Oggi.getDay();
  const NomeGiorno = GiorniDellaSettimana[Indice];
  return NomeGiorno;
}
...
const myObj = {
  name: 'Andrea',
  age: 'ventiCinque', //non ho messo il numero perchè la consegna diceva stringa
  gender: 'M'
};

const result = deleteProp(myObj, 'age');
console.log(result); 
...
function newestMovie(movies) {
  ...
  for (let i = 1; i < movies.length; i++) {
    if (movies[i].Year > newestFilm.Year) {
      newestFilm = movies[i];
    }
  }
...
function searchAndDivide(ricerca) {
  ...
  for (let i = 0; i < movies.length; i++) {
    const movie = movies[i];
    if (movie.Title.toLowerCase().includes(ricerca.toLowerCase())) {
      match.push(movie);
    } else {
      unmatch.push(movie);
    }
